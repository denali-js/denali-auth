    ...
    async authenticate(params) {
      ...
      if (strategies.length !== 0) {
        this.currentUser = await Promise.reduce(strategies, async (user, strategy) => {
          return user || Promise.try(() => strategy.authenticateRequest.call(User, this, params));
        }, null);

        if (!this.currentUser && this.protected !== false) {
          throw new Errors.Unauthorized(`Missing credentials - you must supply credentials for one of these strategies: ${ strategies.map((s) => s.authenticationStrategyName) }`);
        }
      }
    }
