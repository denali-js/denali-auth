...
export default createMixin((MixinBase) => {
  return class AuthenticatableMixin extends MixinBase {
    ...
    static async authenticate(action, params) {
      let failureReason;
      let attemptNextStrategy = async (strategies) => {
        ...
        try {
          ...
          debug(`${ strategy.name } failed with "${ error.message }", ${ strategies.length } strategies remaining`);
          failureReason = failureReason || error;
          return attemptNextStrategy(strategies);
        }
        if (!currentUser) {
          throw new Error(`${ strategy.name }.authenticateRequest() should have returned the current user or errored, but instead it returned ${ currentUser }`);
        }
        debug(`${ strategy.name } succeeded, request is authenticated`);
        return currentUser;
      };
