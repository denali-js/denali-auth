...
import { createMixin, hasMany, Model } from 'denali';
...
export default createMixin((BaseModel: typeof Model, options = {}) => {
  ...
  return class ResetableMixin extends BaseModel {
    ...
    async sendResetPasswordEmail() {
      ...
      let tokens = <Model[]>await this.getPasswordResetTokens();
      ...
      return (<any>this.service('mailer')).send('reset-password', { user: this, token });
    }

    async resetPassword(token: Model, password: string) {
